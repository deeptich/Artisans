...
//Build storage for Images upload using Multer
const storage = multer.diskStorage({
    destination: '../../uploads',
    filename:function(req,res,cb) {
        cb(null,Date.now() + file.file);
    }
});
...
upload(req,res,(err) => {
    if(err){
        res.render('error in upload fucntion')
    }else {
...
router.get('/user/:user_id', async(req,res) => {
    try {
       ...
       ['name','avataar']);
    if(!profile)
        return res.status(400).json({ msg: 'There is no profile for users' });
